...
- ID: ULID `varchar(26)`; timestamp: `timestamptz`; tiền/metric: `numeric`.
...
| `candle` | `candle_id` PK, `provider`, `trading_pair_id` FK, `timeframe`, `open_time`, `close_time`, OHLC và `volume numeric` | UQ provider+pair+timeframe+open_time; CK OHLC/volume không âm, high/low hợp lệ, close_time > open_time; index pair+timeframe+open_time |
| `dataset_version` | `dataset_version_id` PK, `version`, provider/pair/timeframe, `range_start`, `range_end`, `candle_count`, `checksum` | UQ checksum; CK range hợp lệ và count > 0 |
...
| Table | Columns chính | Constraints và indexes |
| --- | --- | --- |
| `experiment` | `experiment_id` PK, `name`, `status`, started/completed time, failure code/message | CK lifecycle status; index status+created_at |
| `experiment_manifest` | `experiment_id` PK/FK, `manifest_version`, `dataset_version_id` FK, strategy kind/ref/version, strategy parameters, backtest/search/evaluation config, optional sentiment config, software version, git commit, fingerprint | UQ fingerprint; CK kind SINGLE/COMPOSITE |
...
| Table | Columns chính | Constraints và indexes |
| --- | --- | --- |
| `news_item` | `news_item_id` PK, source/source item ID, URL, title, optional summary/content, content hash, published/crawled time, analysis status | UQ URL; partial UQ source+source ID; indexes published_at và status+crawled_at |
...
| `idempotency_record` | `scope`, `idempotency_key`, request hash, optional resource type/ID, response status/body, created/expires time | PK scope+key; CK HTTP status 100..599; index expires_at |

Outbox aggregate reference là logic reference vì event có thể thuộc nhiều
schema. Không tạo foreign key đa hình.

## Cần xác nhận trước migration

1. Precision/scale cho price, quantity, fee và metrics.
2. Danh sách timeframe MVP cho CHECK constraint.
3. MVP có user/tenant ownership hay là một workspace dùng chung.
4. Provider News cho phép lưu full content trong bao lâu.
5. Strategy registry snapshot được persist lúc startup hay khi được tham chiếu.

Năm điểm này chưa đổi quan hệ chính, nhưng phải được chốt trước migration đầu.
