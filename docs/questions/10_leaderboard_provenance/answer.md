...
- **Đã nối end-to-end trên code mới nhất:** API có thể tạo Search reproduction từ Experiment gốc khi feature flag reproduction được bật.

Bằng chứng: [`ExperimentController.java`](../../../apps/api/src/main/java/com/cryptostrategy/platform/api/experiment/ExperimentController.java) và [`StartSearchReproductionUseCase.java`](../../../modules/experiment-execution/src/main/java/com/cryptostrategy/platform/execution/api/port/in/StartSearchReproductionUseCase.java).
...
> Từ một dòng Leaderboard, nhóm lần về Evaluation, Backtest Result, Candidate và Manifest. Manifest giữ Dataset/checksum, Strategy/version/parameters, generator/seed và assumptions. Vì vậy Top 1 không chỉ là một con số mà có đầy đủ nguồn gốc để audit và chạy lại.
