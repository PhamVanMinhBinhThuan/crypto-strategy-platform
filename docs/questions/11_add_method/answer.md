...
ADD là vòng lặp, không phải đường thẳng. Sau mỗi quyết định, nhóm đối chiếu lại Quality Attribute Scenario và bằng chứng kiểm thử; phần nào chưa có benchmark thì vẫn ghi là Planned. Mỗi vòng thêm contract và tài liệu, đổi lại quyết định có thể truy ngược về driver cụ thể.

## Cách nói khi trình bày

> Nhóm không chọn Redis hay Worker trước rồi mới tìm lý do. Nhóm bắt đầu từ yêu cầu API không bị chặn khi chạy nhiều Backtest, chọn Queue–Worker, định nghĩa Job/Outbox/idempotency, sau đó dùng test và benchmark để kiểm chứng. Đó là cách ADD nối driver với quyết định và bằng chứng.
