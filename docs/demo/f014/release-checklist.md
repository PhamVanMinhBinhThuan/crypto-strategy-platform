...
| Python 3.11/3.12 | Core Sentiment tests chạy | PASS/PARTIAL | Python 3.12.10, 10 tests pass; TensorFlow optional runtime chưa có |
...
| Sentiment external service | Liveness + readiness, model bundle/token đúng | BLOCKED | Dùng Docker hoặc Python 3.12 ML extra theo runbook |
...
3. Controlled Playwright route interception chứng minh UI/contract, không chứng minh Binance,
   PostgreSQL, Redis transport hoặc external Sentiment runtime.
4. Sentiment core tests không load TensorFlow model; ML demo/measurement nâng cao vẫn `NO_CLAIM`.
