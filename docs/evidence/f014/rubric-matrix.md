...
| 6 | Kiến trúc | Reliability và Observability | 4 | FR-011–FR-016, SC-005 | API / Worker / Web | **PARTIAL** | `failure-recovery.md`: Redis thật reclaim/dedup VERIFIED; external TensorFlow inference và process stop/restart pass; backend/UI recovery pass có kiểm soát | Thiếu authenticated LIVE timeline chứng minh Market/Backtest vẫn chạy trong Sentiment outage |
| 7 | Kiến trúc | Reproducibility và Versioning | 4 | FR-018–FR-021, SC-004, SC-006 | Experiment / Result / Leaderboard | **VERIFIED** | `reproduction.md`: PostgreSQL source/target graph, artifact IDs, immutable snapshot và verdict `MATCHED`; API + controlled browser contract pass | Bổ sung screenshot authenticated Web ở T029 để trình bày, không còn gap persistence |
...
| 17 | Chức năng | Sentiment Analysis | 1 | FR-010, FR-011 | Sentiment / News | **PARTIAL** | 10 Python tests; external TensorFlow model READY và inference pass trước/sau restart; Java state/retry integration; UI controlled pass | Chưa có stored inference result qua Worker/Web trong authenticated LIVE profile |
...
| 23 | Demo | Scenario kiến trúc/xử lý lỗi | 2 | FR-011–FR-021, SC-005–SC-006 | Architecture / API / Worker | **PARTIAL** | Redis/Worker reclaim dùng Redis thật; external Sentiment inference/stop/restart pass; realtime và isolation pass backend + controlled browser | Cần ảnh/timeline authenticated LIVE cho Sentiment outage trong khi Market/Backtest tiếp tục hoạt động |
