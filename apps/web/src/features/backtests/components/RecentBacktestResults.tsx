...
import {
  CandidatePagination,
  CandidateTableShell
} from "@/src/features/experiments/components/CandidateTableParts";
import {
  formatDateTime,
  formatPercent,
  formatScore,
    ...
    <section className="panel history-panel" aria-labelledby="recent-backtests-heading">
      <div className="history-heading">
        <div>
          ...
          <h2 id="recent-backtests-heading">Recent backtest results</h2>
        </div>
      ...
      {!state.loading && !state.error && !items.length && (
        <div className="empty-state">
          ...
          <p>Successful experiment candidates will appear here.</p>
        </div>
      )}
      {items.length > 0 && (
        <>
          <CandidateTableShell label="Recent backtest results">
            <table className="history-table backtest-history-table">
              ...
              <tbody>
                {items.map((item) => (
                  <ResultRow key={item.backtestResultId} item={item} />
                ))}
              </tbody>
            </table>
          </CandidateTableShell>
          <CandidatePagination
            label="Recent backtest results"
            start={start}
            end={end}
            total={total}
    ...
    <tr>
      ...
      <td>
        <Link className="history-primary-link" href={href}>
          {strategyName(item.definition)}
        </Link>
        <small>Candidate #{item.generationIndex + 1}</small>
      </td>
      <td className="numeric">{item.score === null ? "—" : formatScore(item.score)}</td>
      <td
        className={`numeric ${returnValue > 0 ? "metric-positive" : returnValue < 0 ? "metric-negative" : ""}`}
      >
        {formatPercent(item.metrics.totalReturn, true)}
      </td>
      <td className="numeric">{item.metrics.numberOfTrades}</td>
      <td>
        <time dateTime={item.completedAt}>{formatDateTime(item.completedAt)}</time>
      </td>
      <td>
