...
public class MarketDataConfiguration {
    ...
    @Bean MarketDataModuleFactory.Components marketDataModule(MarketDataProvider provider, MarketDataPersistenceFactory.Components persistence) {
        return MarketDataModuleFactory.create(provider, persistence.candles(), persistence.datasets(), persistence.reader(), Clock.systemUTC());
    }
}
