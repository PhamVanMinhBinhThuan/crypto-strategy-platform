...
@Component
public class BacktestJobConsumer {

    private static final Logger log = LoggerFactory.getLogger(BacktestJobConsumer.class);
    ...
    @Scheduled(fixedDelayString = "${worker.consumer.poll-interval-ms:500}")
    public void pollBacktestJobs() {
        String streamKey = workerProperties.streams().getBacktestJobsStream();
        String groupName = workerProperties.consumer().backtestGroup();
        ...
        try {
            ...
            log.error("Error polling backtest jobs from Redis stream '{}': {}", streamKey, ex.getMessage());
        }
    }
}
