...
class BacktestExecutionPipelineIntegrationTest {
    ...
    @Test
    void processesInboundBacktestMessageEndToEnd() throws Exception {
        ...
        MessageEnvelope<BacktestJobPayload> envelope = new MessageEnvelope<>(
                msgId, 1, MessageTypes.BACKTEST_JOB, Instant.now(), "corr-102", payload
        );
        ...
        ExecutionAttempt attempt = ExecutionAttempt.start(
                AttemptId.generate(), new JobId(jId), new CandidateId(candId), 1, "worker-1", Instant.now()
        );
        ...
        BacktestCompletionOutcome outcome = new BacktestCompletionOutcome(
                ...
                result.resultId(), EvaluationResultId.generate(), BigDecimal.valueOf(0.92)
        );
        ...
        verify(candidateEvaluatedPublisher).publishCandidateEvaluated(eq(expId), eq(jId), eq(candId), any(), any(), eq(BigDecimal.valueOf(0.92)), eq("corr-102"));
