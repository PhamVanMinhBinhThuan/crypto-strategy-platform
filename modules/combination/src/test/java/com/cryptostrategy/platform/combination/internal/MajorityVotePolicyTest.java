...
class MajorityVotePolicyTest {
    ...
    @Test void majorityWinsAndTiesHold(){MajorityVotePolicy policy=new MajorityVotePolicy();assertEquals(StrategySignal.BUY,policy.combine(List.of(decision(StrategySignal.BUY),decision(StrategySignal.BUY),decision(StrategySignal.SELL)),StrategyParameterSet.empty()));assertEquals(StrategySignal.HOLD,policy.combine(List.of(decision(StrategySignal.BUY),decision(StrategySignal.SELL)),StrategyParameterSet.empty()));}
