...
import com.cryptostrategy.platform.strategy.internal.fingerprint.DefaultStrategyFingerprintCalculator;
...
public final class StrategyModuleFactory {
    ...
    public static StrategyRegistry registry(List<StrategyPlugin> plugins) {
        return new DefaultStrategyRegistry(plugins);
    }

    public static StrategyFingerprintCalculator fingerprints() {
        return new DefaultStrategyFingerprintCalculator();
    }

    public static StrategyCatalogSynchronization catalogSynchronization(StrategyRegistry registry, StrategyCatalogStore store) {
        return new StrategyCatalogSynchronizer(registry, store)::synchronize;
    }

    public static UserStrategyApplication userStrategies(StrategyRegistry registry, UserStrategyStore store, Clock clock) {
        return new UserStrategyService(registry, store, clock);
    }
}
