...
public final class JdbcUserStrategyStore implements UserStrategyStore {
    ...
    @Override public Optional<UserStrategyVersion> findVersion(UUID owner,UserStrategyVersionId id){try{return versionRows(StrategySql.FIND_VERSION,owner,id.value()).stream().findFirst().map(this::hydrateVersion);}catch(DataAccessException exception){throw errors.translate(exception);}}
    @Override public Optional<UserStrategyVersion> findLatestVersion(UUID owner,UserStrategyId id){try{return versionRows(StrategySql.FIND_LATEST_VERSION,owner,id.value()).stream().findFirst().map(this::hydrateVersion);}catch(DataAccessException exception){throw errors.translate(exception);}}
    @Override public List<UserStrategyVersion> listVersions(UUID owner,UserStrategyId id){try{return versionRows(StrategySql.FIND_VERSIONS,owner,id.value()).stream().map(this::hydrateVersion).toList();}catch(DataAccessException exception){throw errors.translate(exception);}}
    ...
    private List<VersionRow> versionRows(String sql,Object... arguments){return jdbc.query(sql,(rs,row)->new VersionRow(rs.getString("user_strategy_version_id"),rs.getString("user_strategy_id"),rs.getInt("version_no"),rs.getString("strategy_kind"),rs.getString("strategy_version_id"),rs.getString("parameters"),rs.getString("policy_id"),rs.getString("policy_version"),rs.getString("policy_parameters"),rs.getString("lifecycle_status"),rs.getString("fingerprint"),Optional.ofNullable(rs.getTimestamp("published_at")).map(value->value.toInstant()),rs.getTimestamp("created_at").toInstant(),rs.getString("plugin_id"),rs.getString("plugin_version")),arguments);}
    private UserStrategyVersion hydrateVersion(VersionRow row){
        if("SINGLE".equals(row.kind())){
            StrategyReference reference=new StrategyReference(new StrategyVersionId(row.strategyVersionId()),new StrategyPluginId(row.pluginId()),SemanticVersion.parse(row.pluginVersion()));
            return new UserStrategyVersion(new UserStrategyVersionId(row.id()),new UserStrategyId(row.userStrategyId()),row.versionNo(),com.cryptostrategy.platform.strategy.api.model.StrategyKind.SINGLE,new SingleStrategyDraftSource(reference,json.readParameters(row.parameters())),com.cryptostrategy.platform.strategy.api.model.UserStrategyVersionStatus.valueOf(row.status()),row.fingerprint(),row.publishedAt(),row.createdAt());
        }
        List<UserStrategyComponent> components=jdbc.query(FIND_COMPONENTS,(component,index)->new UserStrategyComponent(new StrategyReference(new StrategyVersionId(component.getString("strategy_version_id")),new StrategyPluginId(component.getString("plugin_id")),SemanticVersion.parse(component.getString("version"))),json.readParameters(component.getString("parameters"))),row.id());
        CompositeStrategyDraftSource source=new CompositeStrategyDraftSource(new CombinationPolicyId(row.policyId()),SemanticVersion.parse(row.policyVersion()),json.readParameters(row.policyParameters()),components);
        return new UserStrategyVersion(new UserStrategyVersionId(row.id()),new UserStrategyId(row.userStrategyId()),row.versionNo(),com.cryptostrategy.platform.strategy.api.model.StrategyKind.COMPOSITE,source,com.cryptostrategy.platform.strategy.api.model.UserStrategyVersionStatus.valueOf(row.status()),row.fingerprint(),row.publishedAt(),row.createdAt());
    }
    private record VersionRow(String id,String userStrategyId,int versionNo,String kind,String strategyVersionId,String parameters,String policyId,String policyVersion,String policyParameters,String status,String fingerprint,Optional<Instant> publishedAt,Instant createdAt,String pluginId,String pluginVersion){}
