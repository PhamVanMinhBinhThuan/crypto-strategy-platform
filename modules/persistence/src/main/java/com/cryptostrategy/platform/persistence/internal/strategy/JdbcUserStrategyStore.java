...
public final class JdbcUserStrategyStore implements UserStrategyStore {
    ...
    @Override public Optional<UserStrategyVersion> findLatestVersion(UUID owner,UserStrategyId id){try{return jdbc.query(StrategySql.FIND_LATEST_VERSION,(rs,row)->mapVersion(rs,owner),owner,id.value()).stream().findFirst();}catch(DataAccessException exception){throw errors.translate(exception);}}
